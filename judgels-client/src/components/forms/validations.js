...
export const UserGroup = value =>
  /^[a-zA-Z0-9-]{3,32}$/.test(value) ? undefined : 'Must contain between 3 and 32 alphanumeric characters or dashes';
...
export const Max10Lines = value => {
  return value
    .split('\n')
    .map(s => s.trim())
    .filter(s => s.length > 0).length <= 10
    ? undefined
    : 'Max 10 lines';
};
